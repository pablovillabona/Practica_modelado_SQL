create table "Vehiculos".trabajador(
...
drop table "Vehiculos".trabajador; 
...
alter table "Vehiculos".vehiculo add dni_empleado VARCHAR(10) not null;
alter table "Vehiculos".vehiculo add id_matricula VARCHAR(10) not null;
ALTER TABLE "Vehiculos".vehiculo DROP COLUMN id_matricula;
alter table "Vehiculos".vehiculo add id_matricula int not null;
ALTER TABLE "Vehiculos".vehiculo DROP COLUMN dni_empleado;

create table "Vehiculos".grupo (
	id serial primary key,
	nombre VARCHAR(30) not null
	);
ALTER TABLE "Vehiculos".grupo DROP COLUMN nombre;
alter table "Vehiculos".grupo add nombre VARCHAR(50) not null;
...
create table "Vehiculos".seguro_vehiculo (
	id serial primary key,
...
create table "Vehiculos".compañia_seguro (
	id serial primary key,
	nombre VARCHAR(30) not null
	);
...
create table "Vehiculos".revisiones (
	...
	kilometros_revisión int4 not null,
	id_divisa float4 not null
	);
	
ALTER TABLE "Vehiculos".revisiones DROP COLUMN id_divisa;
ALTER TABLE "Vehiculos".revisiones DROP COLUMN importe_revisión;
alter table "Vehiculos".revisiones add importe_revisión float4 not null;
alter table "Vehiculos".revisiones add id_divisa int not null;

	
create table "Vehiculos".divisa (
	id serial primary key,
	nombre VARCHAR(30) not null
	);
	

alter table "Vehiculos".vehiculo add constraint pk_empleado_vehiculo foreign key (dni_empleado) references "Vehiculos".trabajador(dni);
alter table "Vehiculos".marca add constraint pk_grupo_marca foreign key (id_grupo) references "Vehiculos".grupo(id);
alter table "Vehiculos".modelo add constraint pk_marca_modelo foreign key (id_marca) references "Vehiculos".marca(id);
alter table "Vehiculos".matricula add constraint pk_modelo_matricula foreign key (id_modelo) references "Vehiculos".modelo(id);
alter table "Vehiculos".vehiculo add constraint pk_matricula_vehiculo foreign key (id_matricula) references "Vehiculos".matricula(id);
alter table "Vehiculos".seguro_vehiculo add constraint pk_compañia_seguro foreign key (id_compañia) references "Vehiculos".compañia_seguro(id);
alter table "Vehiculos".seguro_vehiculo add constraint pk_vehiculo_seguro foreign key (id_vehiculo) references "Vehiculos".vehiculo(id);
alter table "Vehiculos".revisiones add constraint pk_vehiculo_revisiones foreign key (id_vehiculo) references "Vehiculos".vehiculo(id);
alter table "Vehiculos".revisiones add constraint pk_divisa_revisiones foreign key (id_divisa) references "Vehiculos".divisa(id);
...
select grupo from "Vehiculos".coches c group by grupo;

insert into "Vehiculos".marca (nombre, id_grupo)
select c.marca, "Vehiculos".grupo.id  
from "Vehiculos".coches c 
inner join "Vehiculos".grupo on c.grupo = "Vehiculos".grupo.nombre 
group by c.grupo, c.marca, "Vehiculos".grupo.id 
order by "Vehiculos".grupo.id, marca;


insert into "Vehiculos".modelo (nombre, id_marca)
select c.modelo, "Vehiculos".marca.id  
from "Vehiculos".coches c 
inner join "Vehiculos".marca on c.marca = "Vehiculos".marca.nombre 
inner join "Vehiculos".grupo on c.grupo = "Vehiculos".grupo.nombre 
group by c.marca, c.modelo, "Vehiculos".marca.id 
order by "Vehiculos".marca.id, modelo;



insert into "Vehiculos".matricula (matricula, id_modelo)
select c.matricula , "Vehiculos".modelo.id  
from "Vehiculos".coches c 
inner join "Vehiculos".modelo on c.modelo = "Vehiculos".modelo.nombre 
inner join "Vehiculos".marca on c.marca = "Vehiculos".marca.nombre 
inner join "Vehiculos".grupo on c.grupo = "Vehiculos".grupo.nombre 
group by c.modelo, c.matricula, "Vehiculos".modelo.id 
order by "Vehiculos".modelo.id, matricula; 




insert into "Vehiculos".vehiculo(color, kilometros_totales, fecha_compra, id_matricula) 
select c.color, c.kms_totales, c.fecha_compra, "Vehiculos".matricula.id  
from "Vehiculos".coches c 
inner join "Vehiculos".matricula on c.matricula = "Vehiculos".matricula.matricula 
inner join "Vehiculos".modelo on c.modelo = "Vehiculos".modelo.nombre 
inner join "Vehiculos".marca on c.marca = "Vehiculos".marca.nombre 
inner join "Vehiculos".grupo on c.grupo = "Vehiculos".grupo.nombre 
group by c.matricula, c.color, c.kms_totales, c.fecha_compra, "Vehiculos".matricula.id 
order by "Vehiculos".matricula.id, color; 

insert into "Vehiculos".compañia_seguro (nombre) 
select aseguradora from "Vehiculos".coches c group by aseguradora;

insert into "Vehiculos".seguro_vehiculo(numero_de_poliza, fecha_vencimiento, id_compañia, id_vehiculo)
select c.n_poliza , c.fecha_alta_seguro, cs.id, "Vehiculos".vehiculo.id 
from "Vehiculos".coches c 
inner join "Vehiculos".compañia_seguro cs on c.aseguradora = cs.nombre
inner join "Vehiculos".matricula on c.matricula = "Vehiculos".matricula.matricula 
inner join "Vehiculos".modelo on c.modelo = "Vehiculos".modelo.nombre 
inner join "Vehiculos".marca on c.marca = "Vehiculos".marca.nombre 
inner join "Vehiculos".grupo on c.grupo = "Vehiculos".grupo.nombre 
inner join "Vehiculos".vehiculo on "Vehiculos".vehiculo.id  = "Vehiculos".matricula.id
group by c.n_poliza, c.fecha_alta_seguro, c.aseguradora, cs.id, "Vehiculos".vehiculo.id
order by aseguradora; 


insert into "Vehiculos".divisa (nombre)
select moneda from "Vehiculos".coches group by moneda;


insert into "Vehiculos".revisiones(fecha_revisión, importe_revisión, kilometros_revisión, id_divisa, id_vehiculo)
select c.fecha_revision, c.importe_revision, c.kms_revision, "Vehiculos".divisa.id, "Vehiculos".vehiculo.id  
from "Vehiculos".coches c 
inner join "Vehiculos".divisa  on c.moneda  = "Vehiculos".divisa.nombre  
inner join "Vehiculos".matricula on c.matricula = "Vehiculos".matricula.matricula 
inner join "Vehiculos".modelo on c.modelo = "Vehiculos".modelo.nombre 
inner join "Vehiculos".marca on c.marca = "Vehiculos".marca.nombre 
inner join "Vehiculos".grupo on c.grupo = "Vehiculos".grupo.nombre 
inner join "Vehiculos".vehiculo on "Vehiculos".vehiculo.id  = "Vehiculos".matricula.id
group by c.fecha_revision, c.importe_revision, c.kms_revision, "Vehiculos".divisa.id, "Vehiculos".vehiculo.id 
order by fecha_revision;  

select "Vehiculos".grupo.nombre grupo, m3.nombre marca, m.nombre modelo, v.fecha_compra, m2.matricula matricula, v.color, v.kilometros_totales, cs.nombre aseguradora, sv.id poliza 
from "Vehiculos".vehiculo v 
inner join "Vehiculos".matricula m2 on m2.id = v.id_matricula 
inner join "Vehiculos".modelo m on m.id = m2.id_modelo  
inner join "Vehiculos".marca m3 on m3.id = m.id_marca 
inner join "Vehiculos".grupo on "Vehiculos".grupo.id = m3.id_grupo 
inner join "Vehiculos".seguro_vehiculo sv on sv.id_vehiculo = v.id 
inner join "Vehiculos".compañia_seguro cs on cs.id = sv.id_compañia 
